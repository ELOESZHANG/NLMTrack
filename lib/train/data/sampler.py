...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            # sample a sequence from the given dataset：返回随机采样的序列id，是否有效，序列中每一帧的信息
            ...
            try:
                ...
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H, W))] * self.num_template_frames    # 构建和图像一样大小的全0mask
                ...
                # self.show(data, 'template',0)
                data = self.processing(data)

                # check whether data is valid， 经过process判断数据是否有效
                valid = data['valid']
            except:
                valid = False
