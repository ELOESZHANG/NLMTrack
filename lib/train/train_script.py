...
from lib.train.actors import NLMTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    config_module.update_config_from_file(settings.cfg_file)  # update cfg from experiments（yaml）
    ...
    # Create network
    if settings.script_name == "nlmtrack":
        net = build_nlmtrack(cfg)
    else:
    ...
    if settings.script_name == "nlmtrack":
        ...
        weight[bins] = 0.01
        weight[bins + 1] = 0.01
        objective = {'giou': giou_loss, 'l1': l1_loss, 'ce': CrossEntropyLoss(weight=weight)}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'ce': cfg.TRAIN.CE_WEIGHT}
        # objective = {'ce': CrossEntropyLoss(weight=weight)}
        # loss_weight = {'ce': cfg.TRAIN.CE_WEIGHT}
        actor = NLMTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
