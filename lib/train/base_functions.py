...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, Imagenet1k, LSOTB_TIR, LSOTB_TIR_VAL, LaSHeR
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LaSHeR", "LSOTB_TIR", "Lsot_TIR", "LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "COCO17", "VID", "TRACKINGNET", "IMAGENET1K"]
        if name == "LSOTB_TIR":
            if settings.use_lmdb:
                print("Building LSOTB_TIR dataset from lmdb")
                # datasets.append(Lasot_lmdb(settings.env.lasot_lmdb_dir, split='train', image_loader=image_loader))
            else:
                datasets.append(LSOTB_TIR(settings.env.lsotb_tir_dir, split='train', image_loader=image_loader))
        if name == "Lsot_TIR":
            if settings.use_lmdb:
                print("Building Lsot_TIR dataset from lmdb")
                # datasets.append(Lasot_lmdb(settings.env.lasot_lmdb_dir, split='train', image_loader=image_loader))
            else:
                datasets.append(LSOTB_TIR_VAL(settings.env.lsot_tir_dir, split='val', image_loader=image_loader))
