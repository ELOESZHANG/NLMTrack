...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        template1 =qkv[:, :512, :, :, :].permute(2, 0, 3, 1, 4)
        # template2 = qkv[:, 256:512, :, :, :].permute(2, 0, 3, 1, 4)
        target = qkv[:, 512:, :, :, :].permute(2, 0, 3, 1, 4)
        qkv = qkv.permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        # asymmetric mixed attention
        # 模板自注意力
        attn = (template1[0] @ template1[1].transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x_mt = (attn @ template1[2]).transpose(1, 2).reshape(B, 512, C)
        #
        # attn2 = (template2[0] @ template2[1].transpose(-2, -1)) * self.scale
        # attn2 = attn2.softmax(dim=-1)
        # attn2 = self.attn_drop(attn2)
        # x_mt2 = (attn2 @ template2[2]).transpose(1, 2).reshape(B, 256, C)

        attn_target = (target[0] @ k.transpose(-2, -1)) * self.scale
        attn_target = attn_target.softmax(dim=-1)
        attn_target = self.attn_drop(attn_target)
        x_s = (attn_target @ v).transpose(1, 2).reshape(B, 256, C)
    ...
    def __init__(self, search_size=384, template_size=192,
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed(
                img_size=search_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)   # 用卷积层（卷积核大小为search_size // patch_size）实现映射
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule，随机深度衰减规律
...
def vit_base_patch16(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)   # partial： 调用函数的一种方式，nn.LayerNorm函数被调用之前eps提前确定了，以便函数能够用更少的参数进行调用
